...
import { chargePorterOrderWallet, refundPorterOrderWallet } from './porter-order-payment.service.js';
...
export async function cancelPorterOrderByUser(userId, orderId, reason, performer = null) {
    ...
    if (refunded.payment.status === PORTER_PAYMENT_STATUS.PAID && refunded.pricing?.total > 0) {
        await refundPorterOrderWallet({
            userId,
            orderId: refunded._id,
            orderNumber: refunded.orderNumber,
            amount: refunded.pricing.total,
            reason,
        });
        refunded.payment.status = PORTER_PAYMENT_STATUS.REFUNDED;
        await refunded.save();
    }
    ...
    const { notifyPorterOrderStatusChange } = await import('./porter-notification.service.js');
...
export async function ratePorterOrder(userId, orderId, { score, comment }) {
    const order = await PorterOrder.findOne({
        _id: orderId,
        userId: new mongoose.Types.ObjectId(userId),
    ...
    order.rating = { score, comment: comment || '' };
    if (order.status === PORTER_ORDER_STATUS.DELIVERED) {
        order.status = PORTER_ORDER_STATUS.COMPLETED;
        order.deliveryState.currentPhase = PORTER_DELIVERY_PHASE.COMPLETED;
